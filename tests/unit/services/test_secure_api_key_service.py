...
class TestArgon2HashVerification(TestSecureAPIKeyService):
    ...
    async def test_verify_key_hash_sha256_rejected(self, api_key_service):
        """Test that SHA256 hashes are rejected for security (no legacy support)."""
        test_key = "vutf_test_key_12345"
        hash_value = hashlib.sha256(test_key.encode()).hexdigest()

        result = await api_key_service._verify_key_hash(test_key, hash_value)
        assert result is False  # SHA256 hashes should be rejected
...
class TestSecureAPIKeyServiceIntegration(TestSecureAPIKeyService):
    ...
    async def test_complete_key_lifecycle_with_legacy_rejection(self, mock_session, tmp_path):
        """Test that legacy SHA256 keys are properly rejected for security."""
        ...
        # Test validation (should reject legacy SHA256 keys for security)
        with patch("asyncio.create_task") as mock_task:
            result = await service.validate_api_key(test_key)

        assert result is None  # Legacy SHA256 keys should be rejected
        # No migration should be scheduled since legacy keys are rejected
        assert mock_task.call_count == 0

        # Legacy migration is no longer supported for security reasons
        # Keys using SHA256 must be regenerated by users

        # Verify no database updates occurred since legacy keys are rejected
