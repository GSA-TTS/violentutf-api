...
repository_operation_total = Counter(
    "repository_operation_total",
    "Total number of repository operations performed",
    ["repository", "operation", "model", "status"],
)

database_query_duration = Histogram(
    "database_query_duration_seconds",
    "Time spent executing database queries",
    ["repository", "model", "query_type"],
)

database_connection_pool_size = Histogram(
...
class BaseRepository(Generic[T]):
    ...
    def __init__(self, session: AsyncSession, model: Optional[Type[T]] = None):
        ...
        self.slow_query_threshold = 0.5  # 500ms threshold for slow queries

    async def _track_repository_operation(
        self, operation_name: str, query_type: str = "read"
    ) -> AsyncContextManager[Any]:
        """
        Context manager for tracking repository operation performance.

        Args:
            operation_name: Name of the repository operation
            query_type: Type of database query (read, write, delete)
        """

        class RepositoryOperationTracker:
            def __init__(self, repo_instance: Any, op_name: str, q_type: str) -> None:
            ...
            async def __aenter__(self) -> "RepositoryOperationTracker":
                self.start_time = time.perf_counter()
                self.operation_id = self.repo.performance_tracker.start_operation(
                    f"{self.repo.repository_name}.{self.operation_name}",
                    metadata={
                        "model": self.repo.model_name,
                        "query_type": self.query_type,
                        "repository": self.repo.repository_name,
                    },
                )
                return self
            ...
            async def __aexit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
                ...
                repository_operation_duration.labels(
                    repository=self.repo.repository_name,
                    operation=self.operation_name,
                    model=self.repo.model_name,
                ).observe(duration)

                repository_operation_total.labels(
                    repository=self.repo.repository_name,
                    operation=self.operation_name,
                    model=self.repo.model_name,
                ...
                # Track slow queries
                if duration > self.repo.slow_query_threshold:
                    ...
                    self.repo.logger.warning(
                        ...
                        operation=self.operation_name,
                        duration=duration,
                        threshold=self.repo.slow_query_threshold,
                    )

                # End performance tracking
                if self.operation_id:
                    await self.repo.performance_tracker.end_operation(self.operation_id, success)  # type: ignore[unreachable]

        return RepositoryOperationTracker(self, operation_name, query_type)
    ...
    async def get_by_id(
        ...
        """
        async with self._track_repository_operation("get_by_id", "read"):
            try:
    ...
    async def update(
        ...
        **kwargs: object,
        ...
        """
        async with self._track_repository_operation("update", "write"):
            try:
                entity_id_str = str(entity_id)
    ...
    async def delete(
        ...
        """
        async with self._track_repository_operation("delete", "delete"):
            try:
                entity_id_str = str(entity_id)
                ...
                if hard_delete:
                    ...
                    if hasattr(self.model, "is_deleted"):
                        ...
                        soft_delete_filters = delete_filters + [
    ...
    async def list_with_pagination(  # noqa: C901
        ...
        """
        async with self._track_repository_operation("list_with_pagination", "read"):
            try:
